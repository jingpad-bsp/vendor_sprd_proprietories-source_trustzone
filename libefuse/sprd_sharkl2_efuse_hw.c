...
int efuse_secure_is_enabled(void) {
    ...
    memset((uint8_t *)&tee_rsp_buf, 0x0, (size_t)(TEE_RSP_LCS_LENGTH + 4));
    ...
    if(1 == tee_rsp.ret_code) {
        ...
        ALOGD("%s: TEECex_SendMsg_To_TEE() error, tee_rsp.ret_code: %d\n", __FUNCTION__,tee_rsp.ret_code);
        return 0;
    }
}
#else
...
int efuse_uid_read_value(unsigned char *uid, int count)
{
    ...
    sprintf(values, "%c%c%c%c%c%c%d%d%d", LOTID_5 + 48, LOTID_4 + 48,
